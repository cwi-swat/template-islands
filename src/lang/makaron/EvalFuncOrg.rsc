...
alias Value = tuple[Path path, OrgValue val];
alias Env = map[str, Value];
alias PEnv = map[Id, Closure];

alias Closure = tuple[{Id ","}* formals, Block body];

alias Result = tuple[PEnv, TString];

alias Path = list[value];

/*
TODO: to make extensible (Monad?)

Define the result ADT + concat

data Result
  = string(str val)
  | orgString(list[Fragment])
  | withPenv(Penv, Result)
  | unit()
  ;
  
then overload/extend concat
- everywhere there's pattern matching it should be made into a function.
...
data Frag
  ...
  | model(Path path, loc org, loc modelOrg, str val)
  ;

alias TString = list[Frag];

TString nil() = [];

TString concat(TString t1, TString t2) = t1 + t2;

str yield(TString t) {
  out = "";
  fs = for (f <- t) {
    switch (f) {
      case string(_, str s): append s;
      case model(_, _, _, str s): append s;
    }
  }
  return ("" | it + f | f <- fs );
}
...
TString run(Text t, map[str, OrgValue] m) = r[1]
...
Result eval(b:(Elt)`{<Elt* es>}`, Env env, PEnv penv) {
  // output all if block is literal
  cloc = b@\loc;
  cloc.length = 1;
  cloc.end.column = cloc.begin.column + 1;
  cloc.end.line = cloc.begin.line;
  out = [string(cloc, "{")];
  for (e <- es) {
    <penv, src> = eval(e, env, penv);
    out = concat(out, src);
  ...
  cloc = b@\loc;
  cloc.offset = cloc.offset + cloc.length - 1;
  cloc.length = 1;
  cloc.begin.column = cloc.end.column;
  cloc.begin.line = cloc.end.line;
  out += [string(cloc, "}")];
  return <penv, out>;
}
...
Result eval((Stat)`$if (<Expr c>) <Block b>`, Env env, PEnv penv) 
  = (<_, <_, true>> := evalExpr(c, env)) ? eval(b, env, penv) : <penv, nil()>;
  
Result eval((Stat)`$if (<Expr c>) <Block b> $else <Block eb>`, Env env, PEnv penv) 
  = (<_, <_, true>> := evalExpr(c, env)) ? eval(b, env, penv) : eval(eb, env, penv);

Result eval((Stat)`$for (<Id x>: <Expr e>) <Block b>`, Env env, PEnv penv)  {
  out = nil();
  if (<Path path, <loc _, list[OrgValue] xs>> := evalExpr(e, env)) {
    println("VALUES = <xs>");
    i = 0;    
    for (v <- xs) {
...
Result eval((Stat)`$<Expr e>;`, Env env, PEnv penv) 
  = <penv, [model(path, e@\loc, v.location, "<v.theValue>")]>
  when <Path path, OrgValue v> := evalExpr(e, env), bprintln("INTER : <v>"), isAtom(v.theValue);
...
Value evalExpr((Expr)`<Expr obj>.<Id f>`, Env env) 
  = <path + ["<f>"], x["<f>"]>
  when bprintln("FIELD: <obj>.<f> = <evalExpr(obj, env)>"), 
  <Path path, <loc mOrg, map[str,OrgValue] x>> := evalExpr(obj, env), 
    bprintln("X = <x>"), x["<f>"]?,
    bprintln("Returning: <x["<f>"]>");
  
default Value evalExpr(Expr e, Env env) = <[], <|file:///-|, "">>;
